...
LearningX 자체 API 경로(`/learningx/api/v1/...`)는 학교·버전마다 다릅니다. 찍어서 맞히려 하면
후보만 늘어나고, 맞는지 확인할 방법도 없습니다.

북마클릿 헤더의 **`⋯`** 버튼이 두 가지를 보여줍니다.

**1. 후보 경로 점검** — 어느 주소가 응답하는지, 몇 개를 돌려주는지, 필드 이름과 항목 제목이 무엇인지.
실패한 것도 이유를 구분해 적습니다(`시간 초과` / `JSON이 아님` / `HTTP 404` / `로그인 페이지로 튕김`).

**2. e-Class가 스스로 부른 주소** — 추측을 끝내는 쪽입니다. 북마클릿이 실행되면 페이지의
`fetch`/`XHR` 를 감싸서, **날짜처럼 생긴 값이 든 JSON 응답**만 골라 적어둡니다. 주차별 학습 페이지를
한 번 열어보면 시청 기한이 어느 주소의 어느 필드에 들어있는지 그대로 드러납니다.

- 응답 **복사본을 읽기만** 하고 원래 흐름은 건드리지 않습니다
- 기록은 그 탭의 `sessionStorage` 에만 남고, 탭을 닫으면 사라집니다
- **우리가 보낸 요청은 제외**합니다(`X-Gwaje-Self` 헤더로 구분). 안 그러면 우리 코드가 부른
  주소가 목록을 덮어버려서 정작 알고 싶은 신호가 묻힙니다
