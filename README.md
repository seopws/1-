...
- **이번 학기만** — 지난 학기 과목은 자동으로 빠집니다. 어느 학기를 보고 있는지 헤더에 표시됩니다
- **과제와 자료를 분리** — 공지·강의자료는 따로 모아 최근 올라온 순으로. 마감일이 있는 것만 과제로 셉니다
- **들어야 할 강의영상 포함** — 주차별 모듈에서 영상을 찾아내고, 시청 완료 여부까지 표시합니다
...
| kind | 무엇 | 날짜의 의미 |
|---|---|---|
| `task` | 과제·퀴즈·시험·토론·출석·**강의영상**, 그리고 실제 마감일이 걸린 모든 것 | 마감일 (`dueAt`) |
| `resource` | 공지·강의자료·페이지 등 마감이 없는 것 | 올라온 날짜 (`postedAt`) |

강의영상은 시청 기한이 안 적혀 있어도 "들어야 하는 것"이라 `task` 로 둡니다. 기한이 없으면
과제 탭의 "기한 없음" 그룹에 들어갑니다.
...
`enrollment_state=active` 로 받아도 지난 학기 과목이 딸려 나오는 경우가 많다. 그래서 과목마다
딸려오는 학기 정보(`include[]=term`)로 이번 학기만 남긴다.

1. 지금 진행 중인 학기(`start_at <= 지금 <= end_at`)를 고른다
2. 여럿이면 가장 늦게 시작한 학기
3. 학기 날짜가 없으면 `enrollment_term_id` 가 큰 쪽 (Canvas 학기 id는 증가한다)

**못 가르겠으면 아무것도 거르지 않는다.** 학기가 하나뿐이거나, 학기 정보가 아예 없거나, 걸렀더니
과목이 하나도 안 남는 경우다. 잘못 걸러서 과제를 숨기는 쪽이 조금 지저분한 쪽보다 훨씬 나쁘다.

고른 학기 이름은 화면 헤더에 적는다. 과목이 안 보일 때 스스로 확인할 수 있어야 한다.

<br>

## 강의영상은 어디서 오나

처음에는 LearningX 고유 경로(`/learningx/api/v1/...`)를 찍어서 맞히려 했다. 학교마다 달라서 맞을
확률이 낮고, 맞는지 확인할 방법도 없었다.

강의영상은 **Canvas 표준 `modules` API**에 들어 있다. LearningX가 주차별 학습활동을 Canvas 모듈로
올리고, 영상은 대개 LTI(`ExternalTool`) 항목으로 끼워 넣기 때문이다. 추측할 필요가 없는 경로다.

```
GET /api/v1/courses/:id/modules?include[]=items
```

핵심은 `completion_requirement` 다. 마감일은 없지만 **"이걸 봐야 한다"와 "봤는지"** 를 알려준다.
`must_view` 가 걸린 항목이 곧 들어야 할 영상이고, `completed` 가 시청 완료 여부다.

담는 규칙:

- `SubHeader` 는 제목일 뿐이라 건너뛴다
- **과제·퀴즈·토론 항목은 일부러 건너뛴다.** 그쪽은 `assignments`/`planner` 에서 제대로 된 마감일과
  함께 이미 가져온다. 여기서 또 담으면 마감일 없는 중복이 생긴다
- 요구사항도 없고 영상처럼 보이지도 않는 항목은 그냥 자료 더미라 담지 않는다
- Canvas는 항목이 많으면 `items` 를 빼고 주므로, 빠진 모듈은 `/modules/:id/items` 로 따로 받아온다

<br>

## 진단 (엔드포인트가 안 맞을 때)

LearningX 자체 API 경로(`/learningx/api/v1/...`)는 학교·버전마다 다릅니다. 그래서 후보를 순서대로
시도하고 실패하면 넘어가는데, 강의영상처럼 그쪽에만 있는 항목이 안 보이면 원인을 알 수가 없습니다.

북마클릿 헤더의 **`⋯`** 버튼을 누르면 어느 엔드포인트가 살아있는지, 몇 개를 돌려주는지, 필드 이름이
무엇인지 한 화면에 보여줍니다. 결과를 복사해서 그대로 붙여넣으면 정확한 경로로 맞출 수 있습니다.
확장 프로그램에는 같은 목적의 **엔드포인트 탐색** 기능이 설정 화면에 있습니다.
